...
(function($) {
    ...
    var methods = {
        ...
        clear: function() {
            ...
            this.each(function() {
                ...
                swfuploadify.queueData.files = null;
    ...
    var handlers = {
        ...
        onSelect: function(file) {
            ...
            for (var n in this.queueData.files) {
                ...
                if (queuedFile.uploaded != true && queuedFile.name == file.name) {
                    ...
                    var replaceQueueItem = confirm('上传列表中已存在文件名为“' + file.name + '”的文件，是否要替换现有文件？');
                    if (!replaceQueueItem) {
            ...
            if ($.inArray('onSelectError', settings.overrideEvents) < 0) {
                switch (errorCode) {
                    case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
                        if (errorMsg > 1) {
                            this.queueData.errorMsg = '最多只能上传 ' + errorMsg + ' 图片.';
                        }
                        else if (errorMsg == 1) {
                            this.queueData.errorMsg = '最多还能上传1张图片.';

                        }
                        else {
                            this.queueData.errorMsg = '你已达到上传的最大张数.';
                        }
                        //                        if (settings.queueSizeLimit > errorMsg) {
                        //                            //this.queueData.errorMsg += '\nThe number of files selected exceeds the remaining upload limit (' + errorMsg + ').';
                        //                            this.queueData.errorMsg = '选择的文件的数量超过了剩余的上传限制 ' + errorMsg + ' 个.';
                        //                        } else {
                        //                            //this.queueData.errorMsg += '\nThe number of files selected exceeds the queue size limit (' + settings.queueSizeLimit + ').';
                        //                            this.queueData.errorMsg = '选择的文件数超过了队列的大小限制 ' + settings.queueSizeLimit + ' 个.';
                        //                        }
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        //this.queueData.errorMsg += '\nThe file "' + file.name + '" exceeds the size limit (' + settings.fileSizeLimit + ').';
                        //this.queueData.errorMsg = '文件“' + file.name + '”大小超出系统限制的 ' + settings.fileSizeLimit + ' 大小.';
                        this.queueData.errorMsg = '只能上传4兆以内的图片!';
                        break;
                    case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
                        //this.queueData.errorMsg += '\nThe file "' + file.name + '" is empty.';
                        //this.queueData.errorMsg = '文件“' + file.name + '”大小异常.';
                        //this.queueData.errorMsg = '不能上传0节字文件!';
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        //this.queueData.errorMsg += '\nThe file "' + file.name + '" is not an accepted file type (' + settings.fileTypeDesc + ').';
                        //this.queueData.errorMsg = '文件“' + file.name + '”类型 ' + settings.fileTypeDesc + ' 不正确.';
                        this.queueData.errorMsg = '不允许上传文件类型的文件!';
                        break;
                }
            }
        ...
        onUploadComplete: function(file) {
            ...
            //alert(this.queueData.queueLength);
