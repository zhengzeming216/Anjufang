...
namespace Common
{
    ...
    /// 没有最后一页的分页控件
    /// </summary>
    public static class PageNoLastControl
    {
        /// <summary>
        /// 
        /// </summary>
        ...
        /// <returns></returns>
        public static string PagerNoLast(this HtmlHelper helper, int numEdge, int numDisplays, int pagesize, int count, int currentPage,
            string linkUrl, string firstText, string prevText, string nextText, string lastText, bool showGo = false)
        {
            ...
            if (!string.IsNullOrEmpty(nextText) && (currentPage < m_totalPage - 1))
            {
                sbHtml.Append(AppendItem(currentPage + 1, m_totalPage, currentPage, nextText + "<em></em>", "next", linkUrl));
            }

            if (!string.IsNullOrEmpty(lastText) && (currentPage < m_totalPage - 1))
            {
